...
def ELBO(x_hat, x, mu, logvar):
    ...
    KLD = -0.0035 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    return MSE + KLD
...
model = RNN_VAE(input_size=3, hidden_size=256, latent_size=18, output_size=1).to(device)
...
torch.save(model.state_dict(), os.path.join(save_dir, 'RNN_VAE_16384_10k.h5'))
...
with torch.no_grad():
    ...
    plt.tight_layout()
    plt.savefig('errorplot_16384_10k.png')
    plt.show()
...
plt.xlabel('Time')
plt.ylabel('Rate')
plt.savefig('benchmark_reconstruction_16384_10k.png')
plt.show()


filename = "loss_16384_10k.txt"
...
plt.grid(True)
plt.show()
plt.savefig('loss_16384_10k.png')
