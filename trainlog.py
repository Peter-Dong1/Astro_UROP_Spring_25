...
def normalize_rate(rate):
    """Normalize rate values to be between 0 and 1"""
    min_val = rate.min()
    max_val = rate.max()
    if torch.allclose(max_val, min_val, rtol=1e-5):
        return rate - min_val, min_val, max_val
    return (rate - min_val) / (max_val - min_val), min_val, max_val

def denormalize_rate(normalized_rate, min_val, max_val):
    """Denormalize rate values back to original scale"""
    if torch.allclose(max_val, min_val, rtol=1e-5):
        return normalized_rate + min_val
    return normalized_rate * (max_val - min_val) + min_val
...
def collate_fn_err(batch):
    ...
    # Normalize rates and store normalization parameters
    normalized_rates = []
    norm_params = []
    for r in rate:
        norm_rate, min_val, max_val = normalize_rate(r)
        normalized_rates.append(norm_rate)
        norm_params.append((min_val, max_val))

    sequences = [torch.stack([r, le, ue], dim=-1) for r, le, ue in zip(normalized_rates, lowErr, upErr)]
    ...
    min_vals = torch.tensor([p[0] for p in norm_params], dtype=torch.float32)
    max_vals = torch.tensor([p[1] for p in norm_params], dtype=torch.float32)

    return x, lengths, min_vals, max_vals
...
def Poisson_NLL(x_hat, x, mu, logvar, min_vals, max_vals):
    """
    Poisson Negative Log-Likelihood (NLL) Loss with KL divergence regularization.
    Works with normalized values and denormalizes before computing the loss.
    """
    # Denormalize the predicted and actual values
    x_hat_denorm = denormalize_rate(x_hat, min_vals.unsqueeze(1), max_vals.unsqueeze(1))
    x_denorm = denormalize_rate(x, min_vals.unsqueeze(1), max_vals.unsqueeze(1))

    # Ensure positive rates
    rate = F.softplus(x_hat_denorm)  # Using softplus to ensure positive values

    # Poisson NLL
    poisson_term = torch.mean(rate - x_denorm * torch.log(rate + 1e-8))

    # KL divergence term
    # kl_term = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp())

    # Total loss with KL weight
    loss = poisson_term #+ 0.1 * kl_term
...
light_curves_sample = load_n_light_curves(3, fits_files, band = "med")
...
# val_loader = DataLoader(val_dataset, batch_size=1, collate_fn=collate_fn_err)


# Initialize the model and optimizer
model = RNN_VAE(input_size=3, hidden_size=300, latent_size=50, output_size=1).to(device)
...
for epoch in range(nepochs):
    ...
    for batch in train_loader:
        x, lengths, min_vals, max_vals = batch
        x = x.to(device)
        ...
        # Extract the rate component
        x_hat_rate = x_hat[..., 0]

        # Compute loss with normalization parameters
        loss = Poisson_NLL(x_hat_rate, x[..., 0], mu, logvar, min_vals, max_vals)
    ...
    with torch.no_grad():
        for batch in train_loader:
            ...
            valid_loss += Poisson_NLL(x_hat_rate, x[..., 0], mu, logvar, min_vals, max_vals).item()  # Compare only with RATE
        # print('1')
    ...
    if epoch % 1000 == 0:
        ...
        x_benchmark, lengths_benchmark, min_vals_benchmark, max_vals_benchmark = collate_fn_err([benchmark_sample])
        x_benchmark = x_benchmark.to(device)
        lengths_benchmark = lengths_benchmark.cpu().to(torch.int64)
        min_vals_benchmark = min_vals_benchmark.to(device)
        max_vals_benchmark = max_vals_benchmark.to(device)
...
with torch.no_grad():
    ...
    max_length = max(len(sample['RATE']) for sample in train_dataset)

    for i in range(10):
        idx = random.randint(0, len(train_dataset) - 1)
        sample = train_dataset[idx]
        x, lengths, min_vals, max_vals = collate_fn_err([sample])
        x = x.to(device)
        lengths = lengths.cpu().to(torch.int64)
        min_vals = min_vals.to(device)
        max_vals = max_vals.to(device)
